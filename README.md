...
- Top-20 appearance counts for one season, with the snapshot denominator. A new season waits until it has 6 stored snapshots. The all-time table is a per-season breakdown plus a total.
...
- Freshness in PT, plus a stale badge when the last successful check is more than 3 hours old.
...
hourly GitHub Action (minute 17, or workflow_dispatch)
  ...
  GET /api/public/v1/leaderboard?limit=100
    ...
    GET /api/public/v1/leaderboard?season=<old>&limit=100  (cursor pages, once)
